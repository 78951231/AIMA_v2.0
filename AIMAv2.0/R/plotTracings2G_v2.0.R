...
plotTracings2G_v2.0 = function(groupAvg_channelTracings,
  ...
  minYaxis_deltaGraph = floor((min(as.numeric(deltaChannel_groups))-15)/10)*10
  ...
  ##############################################################################
  BaselineTilt = as.numeric(avgChannelvalues_groups)
  ...
  barFigure = ggplot(data = df1, aes(x = factor(groups_dat), y = sbp)) +
    ...
    scale_color_manual(values = c('gray80', 'dodgerblue')) +
    theme_prism() +
    ...
    scale_y_continuous(expand = c(0, 0), breaks = seq(0, maxYaxis_barGraph, by = maxYaxis_barGraph/2)) +
  ...
  #df2$groups <- as.factor(df2$groups)
  barDeltaFigure = ggplot(data = df2, aes(x = factor(groups_dat2), y = sbp)) +
  ...
  barDeltaGraph = barDeltaFigure + scale_fill_manual(values = c("grey75", "steelblue")) +
    theme(
      axis.line.x = element_blank(),
      #axis.text.x=element_blank(),
      axis.ticks.x = element_blank(),
